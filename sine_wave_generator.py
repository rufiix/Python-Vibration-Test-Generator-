...
class VibrationTestApp:
    ...
    # ... (wszystkie inne funkcje poza start_playback bez zmian)
    ...
    async def start_playback(self, offset: float = 0.0):
        ...
        # === KLUCZOWA ZMIANA: ROZDZIELENIE LOGIKI ===

        if sine_mode:
            # Dla trybu 'sine' (szybki), możemy od razu ustawić stan i grać
            self.playing = True
            self.paused = False
            self.duration = d
            self.volume = v
            ...
            # Dla trybu 'random' (wolny), najpierw generujemy dane
            ui.notify("Generating random signal...", spinner=True, timeout=10)
            duration_to_generate = self.INFINITE_LOOP_DURATION if d == 0 else d
            generated_noise = await asyncio.to_thread(self.generate_noise_sloped, duration_to_generate)
            
            # Dopiero gdy dane są gotowe, ustawiamy stan i rozpoczynamy odtwarzanie
            self.noise_data = generated_noise
            ...
            self.duration = d
            self.volume = v
            self.start_time = time.time() - offset # Timer startuje TERAZ
            self.elapsed = offset
            self.record_buffer.clear()

        # Wspólna część dla obu trybów - otwarcie strumienia
        if not self.playing: # Sprawdzenie na wypadek anulowania w międzyczasie (choć mało prawdopodobne)
            return
